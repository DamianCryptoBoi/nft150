...
contract MarketV3 is Manager, ERC1155Holder, ERC721Holder, ReentrancyGuard {
	...
	mapping(uint256 => uint256) public orderIdByBidId;
	...
	function createBid(
		...
		orderIdByBidId[totalBids] = _orderId;
		_bidId = totalBids;
	...
	function acceptBid(uint256 _bidId, uint256 _quantity) external whenNotPaused returns (bool) {
		Bid memory bid = bids[_bidId];
		uint256 orderId = orderIdByBidId[_bidId];
		...
		for (uint256 j = startId; j <= endId; j++) {
			...
			if (quantity > 0) {
				...
				if (status) {
					...
					orderIdByBidId[j] = IPolkaMarket(oldMarket).orderIdByVersion(tokenAddress, tokenId, version);
				}
			}
		}
	}
